...
_COMMAND_NAME = "print-insert"


class PrintInsertCommand(BaseCommand):
    ...
    def parser_sub_parser(sub_parsers: argparse._SubParsersAction) -> None:
        parser = sub_parsers.add_parser(_COMMAND_NAME, help="TBD")
    ...
    def handle(
        ...
        sql = self._factory.get_formatter(
            args.dialect, FormatterType.SQL_CREATE_INSERT
        ).format(table)
        print(sql)
